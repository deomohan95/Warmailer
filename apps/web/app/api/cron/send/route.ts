...
function supabaseDb(config: SendConfig) {
  ...
  return {
    ...
    getSequenceSteps: (campaignId: string) => get(`campaign_sequence_steps?campaign_id=eq.${campaignId}&select=*&order=step_order.asc`),
    ...
    getDueCampaignLeads: async (campaignId: string, limit: number, now = new Date()) => {
      const rows = await get<{ id: string; lead_id: string; status: string; next_step_order?: number | null; next_send_at?: string | null; mailbox_id?: string | null }>(
